...
var List = function () {
	...
	var handleList = function() {
        ...
        jqGrid.jqGrid({  
            caption: "用户列表",
            url: hdnContextPath + "/users/list",
            mtype: "post",  
            styleUI: 'Bootstrap',//设置jqgrid的全局样式为bootstrap样式  
            datatype: "json",  
            colNames: ['ID', '头像', '用户名', '昵称', '二维码', '手机唯一标识', '状态'],
            colModel: [  
                ...
                { name: 'qrcode', index: 'age', width: 20, sortable: false ,
                    formatter:function(cellvalue, options, rowObject) {
                        var src = apiServer + cellvalue;
                        var img = "<img src='" + src + "' width='120'></img>"
                        return img;
                ...
                { name: 'cid', index: 'sexValue', width: 20, sortable: false },
                { name: 'isOnline', index: 'province', width: 20, sortable: false, hidden: false,
                    formatter:function(cellvalue, options, rowObject) {
                        if(cellvalue == 0){
                            var font = "<font color='red'>离线</font>";
                            return font;
                        }else if(cellvalue == 1){
                            var font = "<font color='green'>在线</font>";
                            return font;
                        }
                    }
                }
