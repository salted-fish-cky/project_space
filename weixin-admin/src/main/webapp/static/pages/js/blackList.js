var forbidVideo = function(blackListId) {
	
	var flag = window.confirm("取消拉黑");
	if (!flag) {
		return;
	}

	$.ajax({
    	url: $("#hdnContextPath").val() + "/blackList/cancelPullBlack?blackListId=" + blackListId,
    	type: "POST",
    	async: false,
...
var UsersReportsList = function () {
	...
	var handleUsersReportsList = function() {
        ...
        jqGrid.jqGrid({  
            caption: "黑名单列表",
            url: hdnContextPath + "/blackList/getBlackList",
            mtype: "post",  
            styleUI: 'Bootstrap',//设置jqgrid的全局样式为bootstrap样式  
            datatype: "json",  
            colNames: ['ID', '用户名', '头像', '日期',"是否取消拉黑"],
            colModel: [  
                { name: 'id', index: 'id', width: 30, sortable: false, hidden: false },  
                // { name: 'title', index: 'title', width: 30, sortable: false },
                { name: 'username', index: 'content', width: 60, sortable: false },
                ...
                { name: '', index: '', width: 20, sortable: false, hidden: false,
                	formatter:function(cellvalue, options, rowObject) {
                		var button = '<button class="btn btn-outline blue-chambray" id="" onclick=forbidVideo("' + rowObject.id + '") style="padding: 1px 3px 1px 3px;">取消拉黑</button>';
			    		return button;
			    	}
			    }
            ],  
