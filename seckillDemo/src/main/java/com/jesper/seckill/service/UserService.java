...
import com.jesper.seckill.config.Constant;
...
public class UserService {
    ...
    public String login(HttpServletResponse response, LoginVo loginVo) {
        ...
        updateLoginDateById(Long.parseLong(mobile),new Date());
        //生成唯一id作为token
    ...
    public Result regist(LoginVo loginVo){
        ...
        User user = getById(Long.parseLong(mobile));
        if(user != null){
            throw new GlobalException(CodeMsg.MOBILE_IS_REGISTED);
        }
        user = new User();
        user.setId(Long.parseLong(mobile));
        user.setNickname(null);
        user.setPassword( MD5Util.formPassToDBPass(password, Constant.SALTDB));
        user.setHead(null);
        user.setSalt(Constant.SALTDB);
    ...
    private void updateLoginDateById(long id,Date date){
        userMapper.updateLoginDateById(id,date);
    }
